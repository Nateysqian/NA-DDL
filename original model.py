...
class DeepDictionary():
    ...
    # def layer_rec_loss(self,y,rec_data,layer_i):
    #     M=(y-rec_data).T @ (y-rec_data)
    #     u,_,v=torch.linalg.svd(M)
    #     size_p =dd.p_player[layer_i].shape
    #     dd.p_players[layer_i]=u[:size_p[0],:size_p[1]]
    #     loss =torch.trace(self.dictionary_layers[layer_i])
    #     self.dictionary_layers[layer_i]=self.dictionary_layers[layer_i]@self.p_layers[layer_i].T
    #     return loss
...
for layer_i in range(len(dd.dictionary_layers)):
    for epoch in range(epoch_per_level):
        for batch_i, (img_dd, labels) in enumerate(train_loader_dd):
            ...
            img_dd = img_dd.view(batch_size, -1) #40000*784
            ...
            #layer rec loss
            # #layer_rec_lss(self,y,rec_data,layer_i)
            # if layer_i==len(dd.dictionary_layers)-1:
            #     loss =dd.layer_rec_loss(z_prev,z_rec,layer_i)
            #     print("layer_i",layer_i,"  loss :",loss)
...
def show_loss(max_x,max_y,train_data,test_data):
    ...
    #draw line graph
    plt.plot(x,train_data,label="trian",color='red')
...
for epoch in range(epoch_mlp):
    ...
    plt_y_train.append(epoch_total_loss_train)
